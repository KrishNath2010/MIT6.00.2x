...
def generate_models(x, y, degs):
    ...
    #x_copy = [1961, 1962, 1963]
    #y_copy = [4.4, 5.5, 6.6]
    #print("x with len=", len(x), "val=", x)
    #print("y with len=", len(y), "val=", y)
    #print("x_copy with len=", len(x_copy), "val=", x_copy)
    #print("y_copy with len=", len(y_copy), "val=", y_copy)
    answer=[]
    for degree in degs:
        answer.append(np.polyfit(x , y, degree))
    return answer
...
def r_squared(y, estimated):
    ...
    #     error.append((y[x]-estimated[x])**2)
    # for w in error :
    #     sum_error+=w
    # for e in estimated :
    #     sum+=e
    ...
    #     denominator+=g-mean
    # try :
    #     return 1-sum_error/denominator
    # except :
    #     return 1
    error = ((predicted - observed)**2).sum()
    meanError = error/len(observed)
    result= 1 - (meanError/np.var(observed))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        for x_i in x:
            # create yPred
            yPred.append(model[0]*x_i+model[1])
        pylab.plot(x,y,'.b',label = 'aculal values')
        pylab.plot(x,yPred,'-r',label = 'Predicted values')
        pylab.show()
